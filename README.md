...
> I built a local fraud analytics lakehouse using DuckDB, Python, Parquet, and Streamlit. The pipeline ingests the PaySim mobile-money fraud dataset, writes partitioned bronze Parquet, validates records with quality rules, quarantines bad transactions, creates clean silver tables, builds gold fraud marts, and serves an interactive dashboard for fraud KPIs, risk drivers, high-risk accounts, and alert-rule performance.

Good technical points to mention:

- DuckDB can query large CSV and Parquet files locally without a server.
- Bronze, silver, and gold layers separate raw ingestion from trusted analytics.
- Parquet improves local query performance and storage efficiency.
- Quarantine handling is better than silently dropping invalid rows.
- Fraud lift is more useful than raw fraud count because it compares segments to baseline risk.
- Precision and recall show the tradeoff between catching more fraud and creating fewer false alerts.

## Possible Future Improvements

- Add dbt-duckdb models for dependency management and documentation.
- Add GitHub Actions to run tests automatically.
- Add Great Expectations or Soda for richer quality reporting.
- Add customer-network graph analysis.
- Train a simple fraud classification model and compare it against rule-based alerts.
- Add incremental ingestion for new daily transaction files.

## Resume Bullet

Built a local DuckDB lakehouse for mobile-money fraud analytics using 6M synthetic transactions. Implemented raw/bronze/silver/gold layers, Parquet storage, data quality checks, quarantine handling, reusable SQL transformations, pipeline run metadata, and a Streamlit dashboard for fraud KPIs, risk drivers, high-risk account monitoring, and alert-rule evaluation.
